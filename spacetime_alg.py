...
def wedge(q_bold, p_bold):
    ...
    S = np.zeros((6), dtype = complex) # A 6 Dimensional Vector
...
def dot(q_bold, p_bold):
    ...
    if L == 6:
        # if q_bold != p_bold:
        #     raise ValueError('Bivector must be dotted into itself')
        
        return np.dot(Re(q_bold), Re(q_bold)) - np.dot(Im(q_bold), Im(q_bold) )
...
def random_vec():
    ...
    '''
    real_part = 2*np.random.random((4)) - 1
